...
class USBDevice {
public:
    static usb_dev_handle* open(int vendor,
...
private:
    ...
    {
        if (getDeviceIdDescriptor(handle, dev->descriptor.iManufacturer, 0x0409,
                                  devVendor, vendor_size) < 0) {
            std::cerr << "Warning: cannot query manufacturer for device: "
                      << usb_strerror() << "\n";
            usb_close(handle);
            return false;
        }
    ...
    static std::tuple<std::string, std::string> getInfo(struct usb_bus* bus,
                                                        ...
                                                        usb_dev_handle* handle)
    {
        ...
        if (dev->descriptor.idVendor == vendor &&
            dev->descriptor.idProduct == product && canOpenDevice(dev, handle) &&
    ...
    static int getDeviceIdDescriptor(usb_dev_handle* dev, int index, int langid, char* buf, int buflen)
    {
        char buffer[256];
        int rval, i;
